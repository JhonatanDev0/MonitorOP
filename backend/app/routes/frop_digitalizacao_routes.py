...
def buscar_metricas(cd_projeto):
    """Busca as métricas de Frop Digitalização de um projeto específico"""
    try:
        ...
        # Buscar dados da TMP_FROP_DIGITALIZACAO
        query = """
            SELECT
                CD_PROJETO,
                DT_EXPORTACAO,
        ...
        metricas = {
            ...
            'QT_INSTRUMENTO_PREVISTO': row.QT_INSTRUMENTO_PREVISTO or 0,
            'QT_INSTRUMENTO_DIGITALIZADO': row.QT_INSTRUMENTO_DIGITALIZADO or 0,
            'PCT_INSTRUMENTO_DIGITALIZADO': row.PCT_INSTRUMENTO_DIGITALIZADO or '0%'
        }
        ...
        print(f"Erro de banco de dados ao buscar métricas de Frop Digitalização: {str(e)}")
        return jsonify({
            'success': False,
        ...
        print(f"Erro ao buscar métricas de Frop Digitalização: {str(e)}")
        return jsonify({
            'success': False,
            'erro': str(e)
        }), 500
